...
import static com.example.fluxjava.rx2.domain.Constants.*
        ...
        target.toObservable(TodoStore.ListChangeEvent.class)
                .subscribe(
                    new Consumer<TodoStore.ListChangeEvent>() {
                        @Override
                        void accept(TodoStore.ListChangeEvent inListChangeEvent) throws Exception {
        ...
        target.toObservable(TodoStore.ItemChangeEvent.class)
                .subscribe(
                    new Consumer<TodoStore.ItemChangeEvent>() {
                        @Override
                        void accept(TodoStore.ItemChangeEvent inItemChangeEvent) throws Exception {
                            TodoStoreSpec.this.mCountDownLatch.countDown()
                        }
                    })
